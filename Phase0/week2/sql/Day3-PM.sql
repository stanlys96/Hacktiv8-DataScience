...
SELECT t.school, COUNT(c.name) AS total_courses 
FROM teachers t RIGHT JOIN courses c ON t.id = c.teachers_id
GROUP BY t.school;

SELECT t.first_name, t.last_name, COUNT(c.name) AS total_courses 
FROM teachers t LEFT JOIN courses c ON t.id = c.teachers_id
...
SELECT homepage_url, COALESCE(homepage_url, '-')
FROM crunchbase_companies_clean_data;
...
founded_at, 
current_date, 
current_date - founded_at AS company_age 
FROM crunchbase_companies_clean_data;
...
founded_at, 
current_date, 
AGE(current_date, founded_at) AS company_age 
FROM crunchbase_companies_clean_data;
...
SELECT start_station, COUNT(*) AS trip_count FROM dc_bikeshare_q1_2012 GROUP BY start_station ORDER BY trip_count DESC;

SELECT region, SUM(funding_total_usd) AS total_funding
FROM crunchbase_companies_clean_data
GROUP BY region
HAVING SUM(funding_total_usd) > 1000000;
